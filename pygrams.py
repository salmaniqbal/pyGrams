...
def get_args(command_line_arguments):
    ...
    parser.add_argument(
        ...
        "--focus",
        default=None,
        choices=["set", "chi2", "mutual"],
        help=argparse.SUPPRESS,
    )
    parser.add_argument(
        "-fs",
        "--focus_source",
        default="USPTO-random-1000.pkl.bz2",
        help=argparse.SUPPRESS,
    )
    parser.add_argument(
        "-tn",
        "--table_name",
        default=os.path.join("outputs", "table", "table.xlsx"),
        help=argparse.SUPPRESS,
    )

    parser.add_argument(
        "-j", "--json", default=True, action="store_true", help=argparse.SUPPRESS
    )
    ...
    parser.add_argument(
        ...
        "--pick",
        default="sum",
        choices=["median", "max", "sum", "avg"],
        help=argparse.SUPPRESS,
    )
    parser.add_argument(
        "-tst", "--test", default=False, action="store_true", help=argparse.SUPPRESS
    )
    parser.add_argument(
        "-fb",
        "--filter_by",
    ...
    parser.add_argument(
        ...
        "--text_header",
        default="abstract",
        help="the column name for the free text",
    )
    parser.add_argument(
        "-dh", "--date_header", default=None, help="the column name for the date"
    )
    ...
    parser.add_argument(
        "-wt", "--wordcloud_title", default="Popular Terms", help="wordcloud title"
    )

    parser.add_argument(
        "-nltk", "--nltk_path", default=None, help="custom path for NLTK data"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ", ".join(
                [f"{index}. {value}" for index, value in enumerate(predictor_names)]
            )
        )
    ...
    parser.add_argument(
        "-nts", "--nterms", type=int, default=25, help="number of terms to analyse"
    )
    parser.add_argument(
        "-mpq",
        "--minimum-per-quarter",
...
def main(supplied_args):
    ...
    if args.timeseries:
        ...
        for emergence in ["emergent", "declining"]:
            ...
            if args.normalised:
                title = (
                    "Forecasts Evaluation: Normalised Counts"
                    if args.test
                    else "Forecasts: Normalised Counts"
                )
            else:
            ...
            if training_values is not None:
                ...
                filename = os.path.join(
                    dir_path, args.outputs_name + "_" + emergence + "_time_series.csv"
                )
